...
def create_heat_map(data, y_axis, x_axis, color_var, color_scheme):
    heat_map = (
        ...
        .encode(
            ...
            color=alt.Color(
                f"max({color_var}):Q", legend=None, scale=alt.Scale(scheme=color_scheme)
            ),
...
def create_donut(value, label, color):
    ...
    data = pd.DataFrame({"Category": ["", label], "Percentage": [100 - value, value]})
    background = pd.DataFrame({"Category": ["", label], "Percentage": [100, 0]})
...
def compute_population_change(data, target_year):
    ...
    current_year["population_change"] = current_year.population.sub(
        previous_year.population, fill_value=0
    )
    return pd.concat(
        [
            current_year.states,
            current_year.id,
            current_year.population,
            current_year.population_change,
        ],
        axis=1,
    ).sort_values(by="population_change", ascending=False)
...
with columns[0]:
    ...
    if chosen_year > 2010:
        ...
        top_state_change = format_population(
            population_change_data.population_change.iloc[0]
        )
        ...
        bottom_state_change = format_population(
            population_change_data.population_change.iloc[-1]
        )
    else:
    ...
    if chosen_year > 2010:
        growing_states = population_change_data[
            population_change_data.population_change > 50000
        ]
        shrinking_states = population_change_data[
            population_change_data.population_change < -50000
        ]

        growth_percentage = round(
            (len(growing_states) / population_change_data.states.nunique()) * 100
        )
        shrink_percentage = round(
            (len(shrinking_states) / population_change_data.states.nunique()) * 100
        )
...
with columns[1]:
    ...
    choropleth = create_choropleth(
        year_data, "states_code", "population", chosen_palette
    )
    st.plotly_chart(choropleth, use_container_width=True)

    heat_map = create_heat_map(
        population_data, "year", "states", "population", chosen_palette
    )
